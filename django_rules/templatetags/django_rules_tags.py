"""
``django-rules`` template tags. To use in a template just put the following
*load* tag inside a template::

    {% load django_rules_tags %}

"""
from django import template
from django.contrib.auth.models import User, Group, AnonymousUser

from django_rules.exceptions import NotUser
from django_rules.core import ObjectPermissionChecker

register = template.Library()

class ObjectPermissionsNode(template.Node):
    def __init__(self, for_whom, obj, context_var=None, perm=None):
...
@register.tag
def get_obj_perm(parser, token):
    bits = token.split_contents()
    format = '{% get_obj_perm user/group for obj as "context_var" %}'
    if len(bits) != 6 or bits[2] != 'for' or bits[4] != 'as':
        raise template.TemplateSyntaxError("get_obj_perms tag should be in "
            "format: %s" % format)
    ...
    if context_var[0] != context_var[-1] or context_var[0] not in ('"', "'"):
        raise template.TemplateSyntaxError("get_obj_perms tag's context_var "
            "argument should be in quotes")
    context_var = context_var[1:-1]
    return ObjectPermissionsNode(for_whom, obj, context_var, context_var)
